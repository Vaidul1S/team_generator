// generator.dart
//
// Flutter port of the React Native "Generator" screen.
//
// Dependencies to add in pubspec.yaml:
//   shared_preferences: ^2.2.0
//   audioplayers: ^6.0.0
//
// Also register the custom font and the click sound asset:
//   fonts:
//     - family: BrightAura
//       fonts:
//         - asset: assets/fonts/BrightAura.ttf
//   assets:
//     - assets/sounds/click2.mp3
//
// (audioplayers' AssetSource looks inside the "assets/" folder by default,
// so the source path below is "sounds/click2.mp3".)
...
class _GeneratorState extends State<Generator> {
  ...
  List<Player> pool = [];
  bool gt = false; // "generate teams" results visible
  bool mix = false; // "generate mixes" results visible
  List<Player> teams = [];
  int size = 2;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0x8040B0CC),
      body: Stack(
        children: [
          SafeArea(child: _buildMainScreen()),
          if (gt) _buildResultsOverlay(useSize: true, onGenerateAgain: _generateTeams),
          if (mix) _buildResultsOverlay(useSize: false, onGenerateAgain: _generateMixes),
        ],
      ),
    );
  }
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        style: ElevatedButton.styleFrom(
          backgroundColor: const Color(0xFF40B0CC),
